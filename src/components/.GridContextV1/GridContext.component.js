...
export class GridContextComponent extends PureComponent {
  ...
  turnToFreeMode(e) {
    ...
    this.setState({ dropBeforeChild: block });
    ...
    document.body.onmousemove = (e) => {
      ...
      for (let index = 0; index < dragContextPos.length; index++) {
        ...
        if (
          ...
          indexOfFacade = this.createFacade(
            parent.elem,
            facadeBlock,
  ...
  createFacade(parentNode, facadeBlock, children, mousePos, indexOfFacade) {
    const { draggedElem } = this.state;
    const { mouseY } = mousePos;
    ...
    for (let index = 0; index < children.length; index++) {
      ...
      if (
        ...
        facadeBlock !== child &&
        isMouseOn &&
        children[indexOfFacade] !== facadeBlock &&
        mouseY >= childTop &&
        mouseY <= childBottom
      ) {
  ...
  turnToNormalMode(e) {
    const { dropBeforeChild } = this.state;
    const childParent = dropBeforeChild.parentNode;
  ...
  render() {
    ...
    const { isOnFreeMode, dragFrom, dragTo, draggedElem } = this.state;
...
export default GridContextComponent;
